...
bool Client::Start(void) {
  ...
  snprintf(userinfo_raw, sizeof(userinfo_raw) , "%s:%s",
           server_user_.c_str(), server_passwd_.c_str());
  ...
  socket_ = socket_fd;
  thread_ = std::thread(&Client::TheradHandler, this);
...
void Client::TheradHandler(void) {
  int ret;
  char recv_buffer[1024] = {};
  thread_is_running_ = true;
  while (thread_is_running_) {
    memset(recv_buffer, 0x0, sizeof(recv_buffer));
    ret = recv(socket_, recv_buffer, sizeof(recv_buffer), 0);
    if (ret == 0) {
      printf("remote socket close!!!\n");
      break;
    } else if (ret > 0) {
